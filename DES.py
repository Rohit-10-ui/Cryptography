...
def sbox_substitution(bits48):
    ...
    for i in range(8):
        ...
        print(f"S{i+1}: {block} -> {binary}")
        result += binary
    return result

def des_core(block_bin, keys, mode):
    block_bin = permute(block_bin, IP)
    left, right = block_bin[:32], block_bin[32:]
    # Show heading only for encryption
    if mode == "ENCRYPT":
        print("\n========= DES ENCRYPTION =========")
    for i in range(16):
        ...
        new_right = xor(left, pbox_out)
        # ✅ Show only Round 1 & 16 AND only for encryption
        if mode == "ENCRYPT" and (i == 0 or i == 15):
            print(f"\n---- ROUND {i+1} ----")
            print("Left :", left)
            print("Right:", right)
            print("Expanded:", expanded)
            print("After XOR:", xored)
            print("After S-Box:", sbox_out)
            print("After P-Box:", pbox_out)
...
def encrypt(plaintext, key):
    if len(plaintext) != 8 or len(key) != 8:
        print("Plaintext and Key must be exactly 8 characters.")
        return
    plain_bin = text_to_bin(plaintext)
    key_bin = text_to_bin(key)
    keys = generate_keys(key_bin)
    cipher_bin = des_core(plain_bin, keys, "ENCRYPT")
    cipher_text = bin_to_text(cipher_bin)
    print("\nCipher Text:", cipher_text)
    return cipher_text

def decrypt(ciphertext, key):
    cipher_bin = text_to_bin(ciphertext)
    key_bin = text_to_bin(key)
    keys = generate_keys(key_bin)
    keys = keys[::-1]   # reverse order
    plain_bin = des_core(cipher_bin, keys, "DECRYPT")
    print("\nDecrypted Text:", bin_to_text(plain_bin))
